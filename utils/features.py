...
def get_client_features(funel, client) -> pd.DataFrame:
    client['gender']      = client['gender'].fillna('M')
    client['age']         = client['age'].fillna(client['age'].median()).astype(int)
    client['citizenship'] = client['citizenship'].fillna('RUSSIA')
    client['education']   = client['education'].fillna('HIGHER_PROFESSIONAL')
    client['job_type']    = client['job_type'].fillna("USELESS_JOB")
